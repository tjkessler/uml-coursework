...
int main()
{
    ...
    // run Gauss-Siedel for 200 iterations, stopping if error falls below 0.0001
    int ITER = 200;
    float TOL = 0.0001;
    int stop_iter;
    // these are from isolating y w.r.t. y' and y''
    ...
    for (int i = 0; i < ITER; i++)
    {
        ...
        for (int j = 1; j < N - 1; j++)
        {
            // only difference between GS and Jacobi: values used immediately
            f[j] = s1 * f[j - 1] + s2 * f_old[j + 1];
    ...
    FILE *gauss_siedel_res = fopen("gauss_siedel_res.txt", "w");
    for (int i = 0; i < stop_iter; i++)
    {
        fprintf(gauss_siedel_res, "%d %f \n", i, err[i]);
    }
